...
class Signup_model extends CI_Model {
	...
	{
		if(!isset($_POST['name']) || !isset($_POST['gender']) || !isset($_POST['admission_no']) || !isset($_POST['password']) || !isset($_POST['r_password']) || !isset($_POST['email']) || !isset($_POST['mobno']) || !isset($_POST['class']))
		{
			$res=array("error","Fill in all credentials.");
			return $res;
		} 
		if(empty($_POST['name']) || empty($_POST['gender']) || empty($_POST['admission_no']) || empty($_POST['password']) || empty($_POST['r_password']) || empty($_POST['email']) || empty($_POST['mobno']) || empty($_POST['class']))
		{
			$res=array("error","Fill in all credentials");
			return $res;
		}
		...
		$admission_no="V-".strtoupper(trim($_POST['admission_no']));

		$val = $this->db->simple_query('SELECT id, verified, email_verified FROM user_table WHERE admission_no="'.$admission_no.'" AND type="volunteer"');
		if(mysqli_num_rows($val)>0)
		{
			$row=mysqli_fetch_array($val);
			if($row['email_verified']!=1)
			{
				$res=array("error", "<strong>".$admission_no."</strong> is already registered. Please verify your email first. <br/>If you are sure that you haven't registered, please contact placement officer immediately.");
				return $res;
			}
			else if($row['verfied']!=1)
			{
				$res=array("error", "<strong>".$admission_no."</strong> is already registered. Ask the placement officer to verify you. <br/>If you are sure that you haven't registered, please contact placement officer immediately.");
				return $res;
			}
			else
			{
		...
		else
		{
			...
			$val = $this->db->simple_query('SELECT id FROM user_table WHERE email="'.$email.'"  AND type="volunteer"');
			if(mysqli_num_rows($val)>0)
			{
				$res=array("error","Please use your own email. <strong>".$email."</strong> is already registered as a volunteer.");
				return $res;
			}
